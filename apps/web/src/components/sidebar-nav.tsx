...
const MAIN_NAV: NavItem[] = [
  ...
  { labelKey: "nav.orchestrator", href: "/orchestrator", icon: "🧭" },
...
export function SidebarNav() {
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="border-t border-black/10 px-3 py-2 space-y-1">
        <div className="text-[10px] font-medium text-zinc-500">{t("nav.more")}</div>
        <div className="flex flex-wrap gap-x-2 gap-y-0.5 text-[10px]">
          <Link href="/observer" className="text-zinc-600 underline hover:text-zinc-900">
            {t("nav.observer")}
          </Link>
          <Link href="/memory" className="text-zinc-600 underline hover:text-zinc-900">
            {t("nav.memory")}
          </Link>
        </div>
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-700 pt-1">
          <span
