...
export const ADD_TEMPLATES: AddTemplate[] = [
  {
    id: "http_agent",
    title: "HTTP / Webhook Agent（通用）",
    summary:
      "通用接入：填写该 Agent 的 webhook_url，平台经 Bridge 把任务 POST 过去；未填则生成 handoff。适用于任意支持 HTTP 的 Agent。",
    honesty:
      "接入方式：External。通道：Bridge + Webhook + Handoff + Callback。控制深度：Partial（取决于对端 Webhook 行为）。",
    completion_mode: "handoff_callback",
    defaults: {
      display_name: "HTTP Agent（自定义）",
      type: "external",
  ...
  },
  {
    ...
    title: "CLI Agent（通用）",
    summary:
      "通用接入：填写该 Agent 的 cli_path（命令），Bridge 注入 prompt 后执行；未填则生成 handoff。适用于任意命令行 Agent。",
    honesty:
      "接入方式：External / CLI-bridged。通道：Bridge + CLI + Handoff + Callback。控制深度：Partial（取决于命令是否可用）。",
    completion_mode: "handoff_callback",
    defaults: {
      display_name: "CLI Agent（自定义）",
      type: "external",
  ...
  },
  {
    ...
    title: "Codex",
    summary:
      "OpenAI Codex：若本机存在 codex CLI 可由 Bridge 同步执行（codex exec）；否则生成 handoff + callback 说明。",
    honesty:
      "接入方式：External / CLI-bridged。通道：Bridge + CLI + Handoff + Callback。控制深度：Partial（取决于 codex CLI 是否可用）。",
    completion_mode: "handoff_callback",
    defaults: {
      display_name: "Codex（自定义）",
      type: "external",
