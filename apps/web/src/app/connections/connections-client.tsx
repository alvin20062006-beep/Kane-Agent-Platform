...
export type ConnectionsCallbackAudit = {
  kind: string;
  task_id?: string;
  run_id?: string;
  at?: string;
...
type Props = {
  ...
  recentCallbacks?: ConnectionsCallbackAudit[];
};
...
export function ConnectionsClient({
  ...
  return (
    <div className="space-y-4">
      <section
        data-testid="connections-hub-strip"
        className="rounded-lg border border-zinc-200 bg-white p-4"
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1 text-sm">
            <div className="font-semibold">{t("connections.hub.title")}</div>
            <div className={bridgeReachable ? "text-emerald-700" : "text-red-600"}>
              {t("connections.hub.bridge")}: {bridgeReachable ? "online" : "offline"}
            </div>
            {waitingHandoffs > 0 ? (
              <div className="text-sky-700">
                {t("connections.hub.handoffs").replace("{n}", String(waitingHandoffs))}
              </div>
            ) : null}
            {handoffDir ? (
              <div className="text-xs text-zinc-500 font-mono break-all">{handoffDir}</div>
            ) : null}
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              {t("connections.hub.connect_cta")}
            </Link>
            <Link
              href="/local-bridge"
              className="rounded-md border border-zinc-200 px-3 py-1.5 text-xs hover:bg-zinc-50"
            >
              {t("completion.handoff.title")}
            </Link>
          </div>
        </div>
        {recentCallbacks.length > 0 ? (
          <div className="mt-3 border-t border-zinc-100 pt-3">
            <div className="text-xs font-medium text-zinc-600">{t("connections.hub.callback_recent")}</div>
            <ul className="mt-2 space-y-1 text-xs text-zinc-600">
              {recentCallbacks.slice(0, 5).map((item, idx) => (
                <li key={`${item.task_id}-${item.at}-${idx}`}>
                  <span className="font-mono">{item.kind}</span>
                  {item.task_id ? (
                    <>
                      {" · "}
                      <Link href={`/tasks/${item.task_id}`} className="underline">
                        {item.task_id}
                      </Link>
                    </>
                  ) : null}
      ...
      {tab === "adapters" && (
        <div className="rounded-lg border border-zinc-200 bg-white">
          {agents.length === 0 ? (
            ...
            <table data-testid="connections-adapters-table" className="w-full text-sm">
              <thead className="bg-zinc-50 text-zinc-600">
                <tr>
