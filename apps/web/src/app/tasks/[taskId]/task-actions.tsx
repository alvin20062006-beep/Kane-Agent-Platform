...
export function TaskRetryRunButton({
  taskId,
  className,
  testId = "retry-task-button",
}: {
  taskId: string;
  className?: string;
  testId?: string;
  ...
  const run = async () => {
    setBusy(true);
    try {
      await apiPost(`/tasks/${encodeURIComponent(taskId)}/retry`);
      await apiPost(`/tasks/${encodeURIComponent(taskId)}/run`);
  ...
  return (
    <button
      ...
      className={
        className ??
        "rounded-md border border-emerald-200 px-3 py-2 text-sm text-emerald-800 disabled:opacity-50"
      }
    >
    ...
    <div className="rounded-lg border border-zinc-200 bg-white p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <TaskRetryRunButton taskId={taskId} />
          <button
      ...
      {isExternalAgent(selectedAgent) ? (
        <div className="rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-900">
          {t("task_detail.assign_external_hint")}
        </div>
      ) : null}
      {pendingApprovalId ? (
