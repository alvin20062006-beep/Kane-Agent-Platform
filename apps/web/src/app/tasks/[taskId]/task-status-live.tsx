...
export function TaskStatusLive({ taskId, initialTask, onTaskChange }: Props) {
  const [task, setTask] = useState<Task>(initialTask);

  useEffect(() => {
    onTaskChange?.(initialTask);
  }, [initialTask, onTaskChange]);
    ...
    <>
      ...
      {task.needs_attention ? (
        <div
          ...
          className="md:col-span-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"
        >
          <div className="font-medium">Needs attention</div>
          <div className="mt-1">{task.attention_reason || "Operator review is required."}</div>
        </div>
      ) : null}
