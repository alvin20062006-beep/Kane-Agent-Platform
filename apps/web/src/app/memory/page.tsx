...
function MemoryPageInner() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      // Tab counts + status filtering happen client-side, so request the max
      // page (Phase 5 caps lists at 200 by default) to keep counts accurate.
      const data = await apiGet<ListResponse<MemoryItem>>("/memory?limit=1000");
      setAll(data.items);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {!loading && !error && total > all.length && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          {t("memory.truncated_notice")
