...
class DbStore(Generic[T]):
    ...
    def create_if_absent(self, item: T) -> bool:
        ...
        with db.engine.begin() as conn:
            result = conn.execute(
                q,
                {"t": self.entity_type, "id": item_id, "data": json.dumps(payload, ensure_ascii=False)},
            )
    ...
    def compare_and_swap(self, item: T, expected: dict[str, Any]) -> bool:
        ...
        item_id = str(getattr(item, self.id_field))
        payload = item.model_dump()
        where_parts = ["entity_type=:t", "entity_id=:id"]
        params: dict[str, Any] = {
            "t": self.entity_type,
            "id": item_id,
        ...
        for index, (field, value) in enumerate(expected.items()):
            ...
            where_parts.append(f"COALESCE(data->>'{field}', '') = COALESCE(:{key}, '')")
            params[key] = "" if value is None else str(value)
        q = text(
            f"""
            UPDATE octopus_entities
