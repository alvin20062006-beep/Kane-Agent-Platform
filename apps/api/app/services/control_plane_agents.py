...
def _default_capabilities(adapter_id: str) -> AgentCapabilities:
    ...
    if adapter_id in ("http_agent", "cli_agent"):
        return AgentCapabilities(
            can_chat=adapter_id == "http_agent",
            can_code=True,
            can_browse=adapter_id == "http_agent",
            can_use_skills=False,
...
def patch_control_plane_agent(agent_id: str, body: AgentPatchBody) -> Agent:
    ...
    if body.bound_skill_ids is not None:
        data["bound_skill_ids"] = list(body.bound_skill_ids)
...
def _test_description(adapter_id: str) -> str:
    ...
    if adapter_id == "http_agent":
        return "HTTP agent test: Bridge will POST to webhook_url if set, otherwise handoff + callback."
    if adapter_id == "cli_agent":
        return "CLI agent test: Bridge runs cli_path if set, otherwise handoff + callback."
