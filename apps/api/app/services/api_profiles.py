...
def delete_profile(profile_id: str) -> dict[str, Any]:
    """
    Delete an API profile and clean up any agent bindings pointing to it.
    This keeps the system consistent when users remove a misconfigured profile.
    """
    existing = api_profiles_repo.get(profile_id)
    if not existing:
        raise HTTPException(status_code=404, detail="profile_not_found")

    # Remove bindings referencing this profile to avoid dangling references.
    bindings = [b for b in api_bindings_repo.list() if b.profile_id == profile_id]
    for b in bindings:
        api_bindings_repo.delete(b.binding_id)

    api_profiles_repo.delete(profile_id)
    return {"deleted": True, "profile_id": profile_id, "bindings_deleted": len(bindings)}


def test_profile_connectivity(profile_id: str) -> dict[str, Any]:
    """Lightweight LLM ping for Settings connectivity check."""
    from .llm_client import LLMNotConfiguredError, resolve_llm_api_key

    p = api_profiles_repo.get(profile_id)
    if not p:
        raise HTTPException(status_code=404, detail="profile_not_found")
    api_key = resolve_llm_api_key(p)
    if not api_key:
        return {
            "ok": False,
            "profile_id": profile_id,
            "error": "api_key_not_configured",
            "hint": "Set OCTOPUS_LLM_API_KEY or provide api_key on the profile.",
        }
    import httpx
    ...
    try:
        ...
        return {"ok": True, "profile_id": profile_id, "status_code": r.status_code}
    except LLMNotConfiguredError:
        return {"ok": False, "profile_id": profile_id, "error": "llm_not_configured"}
